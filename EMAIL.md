...
- Clicking it opens the Email progress screen and sends each generated PDF to the matching XLSX entry.
...
- State: `composeApp/src/commonMain/kotlin/com/cmm/certificates/feature/settings/SmtpSettingsStore.kt`
...
The output directory and `docIdStart` are stored in `ConversionProgressStore` when PDFs are generated.
...
- DataStore Preferences (KMP): `androidx.datastore:datastore` and `androidx.datastore:datastore-preferences`.

## Platform Support
- JVM: supported (Simple Java Mail implementation).
- Android/iOS: not supported yet (throws `UnsupportedOperationException`).
  - See `composeApp/src/androidMain/.../SmtpClient.android.kt` and `composeApp/src/iosMain/.../SmtpClient.ios.kt`.
