...
This file is guidance for the repository rather than a claim that every screen already matches these rules exactly. In this repo, presentation code now lives under `feature/**/presentation`, with feature-local components under `feature/**/presentation/components` and route entry providers under `feature/**/presentation/navigation`.
