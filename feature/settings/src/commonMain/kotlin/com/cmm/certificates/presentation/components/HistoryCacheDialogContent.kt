...
@Composable
internal fun SentHistoryPage(sentHistory: List<SentEmailRecord>) {
    ...
    HistoryPageLayout(
        ...
        extraSummary = if (dateQuery.isNotBlank()) {
            stringResource(
        ...
        extraSummaryColor = MaterialTheme.colorScheme.primary,
        emptyStateText = stringResource(Res.string.settings_history_sent_empty),
        query = query,
        dateQuery = dateQuery,
...
internal fun CachedEmailsPage(
    ...
    HistoryPageLayout(
        ...
        filteredCount = filtered.size,
        extraSummary = cachedLastReason?.let { reason ->
            "${stringResource(Res.string.settings_history_reason_label)}: ${resolveEmailStopReason(reason)}"
        },
        extraSummaryColor = MaterialTheme.colorScheme.error,
        emptyStateText = stringResource(Res.string.settings_history_cached_empty),
        query = query,
        dateQuery = dateQuery,
        onQueryChange = { query = it },
        onDateChange = { dateQuery = it },
...
private fun HistoryPageLayout(
    ...
    filteredCount: Int,
    extraSummary: String?,
    extraSummaryColor: androidx.compose.ui.graphics.Color,
    ...
    Column(
        ...
        if (!extraSummary.isNullOrBlank()) {
            Text(
                text = extraSummary,
                style = MaterialTheme.typography.labelMedium,
                color = extraSummaryColor,
            )
        }
        if (filteredCount == 0) {
            EmptyHistoryState(emptyStateText)
        } else {
            LazyColumn(
                modifier = Modifier.fillMaxSize(),
                verticalArrangement = Arrangement.spacedBy(Grid.x4),
